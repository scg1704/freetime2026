/**
 * GoogleButton.jsx
 *
 * Reusable "Continue with Google" button.
 * Uses @react-oauth/google under the hood.
 *
 * Install once in your project:
 *   npm install @react-oauth/google
 *
 * Props:
 *   onSuccess(googleUser) – called with decoded Google profile
 *   label                 – button text (default: "Continuar con Google")
 *   disabled              – greys out the button
 */
import { useGoogleLogin } from '@react-oauth/google';
import { useState } from 'react';

// Google's userinfo endpoint – returns name, email, picture, sub (Google ID)
async function fetchGoogleProfile(accessToken) {
  const res = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (!res.ok) throw new Error('No se pudo obtener el perfil de Google.');
  return res.json();
}

export default function GoogleButton({ onSuccess, label = 'Continuar con Google', disabled = false }) {
  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState('');

  const handleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setLoading(true);
      setError('');
      try {
        const profile = await fetchGoogleProfile(tokenResponse.access_token);
        // profile shape: { sub, name, given_name, family_name, email, picture, email_verified }
        await onSuccess(profile);
      } catch (err) {
        setError(err.message || 'Error al conectar con Google.');
      } finally {
        setLoading(false);
      }
    },
    onError: () => {
      setError('Google canceló el inicio de sesión.');
      setLoading(false);
    },
  });

  return (
    <div className="space-y-2">
      <button
        type="button"
        onClick={() => { setError(''); handleLogin(); }}
        disabled={disabled || loading}
        className="
          w-full flex items-center justify-center gap-3
          bg-white border-2 border-gray-200
          hover:border-gray-300 hover:bg-gray-50
          active:scale-[0.98]
          disabled:opacity-50 disabled:cursor-not-allowed
          rounded-2xl py-4 px-6
          font-bold text-gray-700 text-sm
          transition-all shadow-sm
        "
      >
        {loading ? (
          /* Spinner */
          <svg className="w-5 h-5 animate-spin text-gray-400" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
          </svg>
        ) : (
          /* Google "G" logo SVG – no external dependency */
          <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
        )}
        {loading ? 'Conectando...' : label}
      </button>

      {error && (
        <p className="text-xs text-red-500 font-medium text-center">{error}</p>
      )}
    </div>
  );
}