...
export default function TaskDetailsPage() {
  ...
  if (!task) return <div className="flex items-center justify-center min-h-screen">Tarea no encontrada</div>;
  ...
  return (
    <div className="px-6 py-8 space-y-8 bg-white min-h-screen">
      ...
      <div className="grid grid-cols-2 gap-4">
        <StatItem icon={<DollarSign className="text-green-500" />} label="Presupuesto" value={`$${task.budget?.toLocaleString()}`} />
        <StatItem icon={<Clock className="text-blue-500" />} label="Duración" value={`${task.estimatedDuration} Horas`} />
        <StatItem icon={<Calendar className="text-orange-500" />} label="Fecha" value={new Date(task.date).toLocaleDateString()} />
        <StatItem icon={<Award className="text-purple-500" />} label="Nivel" value={task.specializationLevel} />
      </div>

      <section className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500">Descripción</h2>
        <p className="text-secondary leading-relaxed">{task.description}</p>
      </section>

      <section className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500">Publicado por</h2>
        <div className="flex items-center justify-between p-6 bg-gray-50 rounded-[32px] border border-gray-100">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gray-200 rounded-2xl overflow-hidden">
              <img src={`https://picsum.photos/seed/${task.fulltimerId}/100`} alt={task.fulltimerName} referrerPolicy="no-referrer" />
            </div>
            <div>
      ...
      <div className="fixed bottom-24 left-6 right-6 flex gap-4">
        ...
        <button
          onClick={() => navigate(`/freetimer/execution/${task.id}`)}
