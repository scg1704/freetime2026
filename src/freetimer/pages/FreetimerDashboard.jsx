...
import { Search, Clock, CheckCircle2, XCircle, AlertCircle, PlayCircle } from 'lucide-react';
import { useAuth } from '../../shared/context/AuthContext';
import { useNavigate } from 'react-router-dom';

export default function FreetimerDashboard() {
  const { user } = useAuth();
    ...
    <div className="px-6 py-8 space-y-10">
      ...
      <div className="space-y-1">
        ...
        <p className="text-secondary font-medium">
          {apiError
            ...
            : `${tasks.filter((t) => t.status === 'PENDING').length} postulaciones esperando respuesta`}
        </p>
      </div>
      ...
      <section className="space-y-4">
        ...
        {loading ? (
          ...
          <motion.div
            ...
            onClick={() => navigate(`/freetimer/execution/${nextTask.id}`)}
          ...
          <div className="bg-gray-50 p-12 rounded-[40px] border-2 border-dashed border-gray-200 text-center space-y-4">
            ...
            <button onClick={() => navigate('/freetimer/tasks')} className="text-primary font-bold hover:underline">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatusCard icon={<Clock className="text-yellow-500" />} label="Esperando" count={tasks.filter((t) => t.status === 'PENDING').length} />
      ...
      <div className="fixed bottom-24 right-6 z-40">
        <button
          onClick={() => navigate('/freetimer/tasks')}
      ...
      {/* Actividad reciente */}
      <section className="space-y-4">
        <h2 className="text-sm font-bold uppercase tracking-widest text-gray-500">Actividad Reciente</h2>
        {tasks.length > 0 ? (
          <div className="space-y-4">
            {tasks.map((task) => (
              <div key={task.id} className="bg-white p-5 rounded-3xl border border-gray-100 flex items-center justify-between hover:border-primary/20 transition-colors">
                <div>
                  <h4 className="font-bold">{task.title}</h4>
                  <p className="text-secondary text-sm">{task.fulltimerName} • {task.status}</p>
                </div>
          ...
          <p className="text-center text-secondary py-8">No hay actividad reciente.</p>
        )}
      </section>
    </div>
  );
